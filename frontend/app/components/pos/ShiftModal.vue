<template>
  <Dialog :open="modelValue" @update:open="$emit('update:modelValue', $event)">
    <DialogContent class="sm:max-w-sm bg-neutral-800 border-neutral-700">
      <DialogHeader>
        <DialogTitle class="text-xl font-black text-white uppercase tracking-tighter">
      ...
      <DialogFooter>
        <DialogClose as-child>
          <Button variant="outline" class="border-neutral-600 text-neutral-400 hover:text-white">
            Cancel
          </Button>
        </DialogClose>
        <Button
          :disabled="loading || shiftAmount <= 0"
          :class="[
            loading ? 'opacity-50 cursor-not-allowed' : '',
            shiftAction === 'OPEN' ? 'bg-success-600 hover:bg-success-500' : 'bg-red-600 hover:bg-red-500',
          ]"
...
const performAction = async () => {
    ...
    try {
        ...
        error.value = e.message || "Failed to update shift"
    } finally {
        loading.value = false
    }
}
